...
function configController(): void
{
    requireAuth();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($acao === 'atualizar_perfil') {
            handleUpdateProfile($id_usuario, $tipo);
        } elseif ($acao === 'alterar_senha') {
            handleChangePassword($id_usuario);
        } elseif ($acao === 'alterar_avatar') {
            handleChangeAvatar($id_usuario);
        }
    }

    $data = loadConfigData($id_usuario, $tipo);
    render('configView', 'Configurações', $data);
}
...
function handleChangeAvatar(int $id_usuario): void
{
    ...
    if ($_FILES['avatar']['error'] !== UPLOAD_ERR_OK) {
        ...
        error_log("Avatar upload error: " . json_encode(['error_code' => $errorCode]));
