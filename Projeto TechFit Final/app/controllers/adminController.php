...
function adminController(): void
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        switch ($action) {
            ...
            case 'add_funcionario':
                require_once __DIR__ . '/../helpers/funcionarioHelper.php';
                $tab = 'funcionarios';
                $nomeFun = trim($_POST['nome_func'] ?? '');
                $cpfFun = preg_replace('/[^0-9]/', '', $_POST['cpf_func'] ?? '');
                $salario = isset($_POST['salario']) ? (float) $_POST['salario'] : 0;
                $cargo = trim($_POST['cargo'] ?? '');
                $cargaHoraria = isset($_POST['carga_horaria']) ? (int) $_POST['carga_horaria'] : 0;
                $idUsuarioFun = isset($_POST['id_usuario_func']) ? (int) $_POST['id_usuario_func'] : 0;

                if ($nomeFun === '' || $cpfFun === '' || $salario <= 0 || $cargo === '') {
                    flash('Nome, CPF, salário e cargo são obrigatórios', 'error');
                } else {
                    try {
                        $pdo = Connect::conectar();
                        
                        // Se não houver ID de usuário, cria automaticamente
                        if ($idUsuarioFun <= 0) {
                            $idUsuarioFun = criarUsuarioFuncionario($nomeFun, $cpfFun, $cargo);
                            if ($idUsuarioFun === null) {
                                flash('Erro ao criar usuário automático para funcionário', 'error');
                                break;
                            }
                        }
                        
                        $stmt = $pdo->prepare('INSERT INTO Funcionarios (salario, carga_horaria, cargo, id_usuario) 
                                             VALUES (:salario, :carga, :cargo, :id_usuario)');
                        $stmt->execute([
                            ':salario' => $salario,
                            ':carga' => $cargaHoraria,
                            ':cargo' => $cargo,
                            ':id_usuario' => $idUsuarioFun
                        ]);
                        flash('Funcionário adicionado com sucesso!', 'success');
                    } catch (Exception $e) {
                        flash('Erro ao adicionar funcionário: ' . $e->getMessage(), 'error');
                    }
                }
                break;
            ...
            case 'edit_funcionario':
                $tab = 'funcionarios';
                $idFunc = (int) ($_POST['id_funcionario'] ?? 0);
                $salario = isset($_POST['salario']) ? (float) $_POST['salario'] : 0;
                $cargaHoraria = isset($_POST['carga_horaria']) ? (int) $_POST['carga_horaria'] : 0;
                $cargo = trim($_POST['cargo'] ?? '');
                $idUsuarioFun = isset($_POST['id_usuario_func']) ? (int) $_POST['id_usuario_func'] : 0;

                if ($idFunc <= 0 || $salario <= 0 || $cargo === '' || $idUsuarioFun <= 0) {
                    flash('Dados inválidos para edição', 'error');
                } else {
                    try {
                        $pdo = Connect::conectar();
                        $stmt = $pdo->prepare('UPDATE Funcionarios SET salario = :salario, carga_horaria = :carga, 
                                             cargo = :cargo, id_usuario = :id_usuario WHERE id_funcionario = :id');
                        $stmt->execute([
                            ':salario' => $salario,
                            ':carga' => $cargaHoraria,
                            ':cargo' => $cargo,
                        ...
                        flash('Erro ao atualizar funcionário: ' . $e->getMessage(), 'error');
                    }
                }
                break;
            ...
            case 'delete_funcionario':
                ...
                if ($idFunc <= 0) {
                    ...
                    try {
                        ...
                        flash('Erro ao remover funcionário: ' . $e->getMessage(), 'error');
                    }
                }
                break;
            ...
            case 'edit_user':
                ...
                $enderecoAlunoEdit = trim($_POST['edit_endereco_aluno'] ?? '');
    ...
    $funcNomeFiltro = trim($_GET['f_func_nome'] ?? '');
    $funcCargoFiltro = trim($_GET['f_func_cargo'] ?? '');
    
    // buscar funcionarios com filtros
    $sqlFuncionarios = 'SELECT f.id_funcionario, f.salario, f.carga_horaria, f.cargo, f.id_usuario, u.nome, u.cpf
                        FROM Funcionarios f
    ...
    if ($funcNomeFiltro !== '') {
        $sqlFuncionarios .= ' AND u.nome LIKE :nome_func';
        $paramsFuncionarios[':nome_func'] = '%' . $funcNomeFiltro . '%';
    }
    
    if ($funcCargoFiltro !== '') {
        $sqlFuncionarios .= ' AND f.cargo LIKE :cargo_func';
        $paramsFuncionarios[':cargo_func'] = '%' . $funcCargoFiltro . '%';
    }
    
    $sqlFuncionarios .= ' ORDER BY u.nome';
    
    $stmtF = $pdo->prepare($sqlFuncionarios);
    $stmtF->execute($paramsFuncionarios);
    ...
    // Pagamentos
    $pagAlunoFiltro = trim($_GET['f_pag_aluno'] ?? '');
    $pagStatusFiltro = trim($_GET['f_pag_status'] ?? '');
    $pagMetodoFiltro = trim($_GET['f_pag_metodo'] ?? '');
    
    try {
        $pdo = Connect::conectar();
        $sqlPagamentos = 'SELECT p.id_pagamento, p.status, p.valor, p.metodo_pagamento, p.data_pagamento, a.id_aluno, u.nome as nome_aluno, u.cpf FROM Pagamentos p JOIN Alunos a ON p.id_aluno = a.id_aluno JOIN Usuarios u ON a.id_usuario = u.id_usuario WHERE 1=1';
        $paramsPagamentos = [];
        
        if ($pagAlunoFiltro !== '') {
            $sqlPagamentos .= ' AND u.nome LIKE :nome_aluno';
            $paramsPagamentos[':nome_aluno'] = '%' . $pagAlunoFiltro . '%';
        }
        
        if ($pagStatusFiltro !== '') {
            $sqlPagamentos .= ' AND p.status = :status';
            $paramsPagamentos[':status'] = $pagStatusFiltro;
        }
        
        if ($pagMetodoFiltro !== '') {
            $sqlPagamentos .= ' AND p.metodo_pagamento = :metodo';
            $paramsPagamentos[':metodo'] = $pagMetodoFiltro;
        }
        
        $sqlPagamentos .= ' ORDER BY p.data_pagamento DESC';
        
        $stmtPag = $pdo->prepare($sqlPagamentos);
        $stmtPag->execute($paramsPagamentos);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        switch ($action) {
            case 'edit_pagamento':
                $pag_id = isset($_POST['id_pagamento']) ? (int)$_POST['id_pagamento'] : 0;
                $novo_status = trim($_POST['status'] ?? 'pendente');
                
                if ($pag_id > 0) {
                    try {
                        $pdo = Connect::conectar();
                        $stmt = $pdo->prepare('UPDATE Pagamentos SET status = ? WHERE id_pagamento = ?');
                        $stmt->execute([$novo_status, $pag_id]);
                        flash('success', 'Status do pagamento atualizado com sucesso!');
                    } catch (Exception $e) {
                        flash('error', 'Erro ao atualizar pagamento: ' . $e->getMessage());
                    }
                }
                header('Location: ' . $_SERVER['PHP_SELF'] . '?active_tab=pagamentos');
                exit;
            ...
            case 'delete_pagamento':
                ...
                if ($pag_id > 0) {
                    try {
                        ...
                        flash('error', 'Erro ao remover pagamento: ' . $e->getMessage());
                    }
                }
                header('Location: ' . $_SERVER['PHP_SELF'] . '?active_tab=pagamentos');
                exit;
            ...
            case 'delete_ticket':
                ...
                if (!empty($ticket_id)) {
                    try {
                        $pdo = Connect::conectar();
                        $stmt = $pdo->prepare('DELETE FROM Suporte WHERE ticket = ?');
                        $stmt->execute([$ticket_id]);
                        flash('success', 'Ticket removido com sucesso!');
                    } catch (Exception $e) {
                        flash('error', 'Erro ao remover ticket: ' . $e->getMessage());
                    }
                }
                header('Location: ' . $_SERVER['PHP_SELF'] . '?active_tab=tickets');
                exit;
        }
    }

    // Tickets
    $ticketAlunoFiltro = trim($_GET['f_ticket_aluno'] ?? '');
    $ticketStatusFiltro = trim($_GET['f_ticket_status'] ?? '');
    $ticketCategoriaFiltro = trim($_GET['f_ticket_categoria'] ?? '');
    
    try {
        $pdo = Connect::conectar();
        $sqlTickets = 'SELECT s.ticket, s.status, s.categoria_suporte, s.descricao_suporte, u.nome FROM Suporte s JOIN Alunos a ON s.id_aluno = a.id_aluno JOIN Usuarios u ON a.id_usuario = u.id_usuario WHERE 1=1';
        $paramsTickets = [];
        
        if ($ticketAlunoFiltro !== '') {
            $sqlTickets .= ' AND u.nome LIKE :nome_aluno';
            $paramsTickets[':nome_aluno'] = '%' . $ticketAlunoFiltro . '%';
        }
        
        if ($ticketStatusFiltro !== '') {
            $sqlTickets .= ' AND s.status = :status';
            $paramsTickets[':status'] = $ticketStatusFiltro;
        }
        
        if ($ticketCategoriaFiltro !== '') {
            $sqlTickets .= ' AND s.categoria_suporte LIKE :categoria';
            $paramsTickets[':categoria'] = '%' . $ticketCategoriaFiltro . '%';
        }
        
        $sqlTickets .= ' ORDER BY s.ticket DESC';
        
        $stmtTick = $pdo->prepare($sqlTickets);
        $stmtTick->execute($paramsTickets);
    ...
    $data = [
        ...
        'f_nome'              => $nomeFiltro,
        'f_tipo'              => $tipoFiltro,
        'f_func_nome'         => $funcNomeFiltro,
        'f_func_cargo'        => $funcCargoFiltro,
        ...
        'f_pag_aluno'         => $pagAlunoFiltro,
        'f_pag_status'        => $pagStatusFiltro,
        'f_pag_metodo'        => $pagMetodoFiltro,
        'f_ticket_aluno'      => $ticketAlunoFiltro,
        'f_ticket_status'     => $ticketStatusFiltro,
        'f_ticket_categoria'  => $ticketCategoriaFiltro,
    ];
