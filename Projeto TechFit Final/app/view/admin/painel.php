...
<div class="container my-4">
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane fade p-3 bg-white rounded shadow-sm" id="funcionarios" role="tabpanel">
            ...
            <div class="mb-4">
                ...
                <form method="post" class="row g-2">
                    ...
                    <div class="col-md-3"><input name="nome_func" class="form-control" placeholder="Nome (informativo)" required></div>
                    <div class="col-md-3"><input name="cpf_func" class="form-control" placeholder="CPF" required></div>
                    <div class="col-md-2"><input name="salario" type="number" step="0.01" class="form-control" placeholder="Salário" required></div>
                    <div class="col-md-2"><input name="cargo" class="form-control" placeholder="Cargo" required></div>
                    <div class="col-md-2"><input name="carga_horaria" type="number" class="form-control" placeholder="Carga horária (ex: 40)" required></div>
                    <div class="col-md-2"><input name="id_usuario_func" type="number" class="form-control" placeholder="ID usuário (opcional)"></div>
                    <div class="col-12 mt-2"><button class="btn btn-primary">Adicionar</button></div>
                </form>
            </div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form method="post">
                            ...
                            <div class="modal-body">
                                <div class="mb-2"><label>Salário</label><input id="edit_salario" name="salario" type="number" step="0.01" class="form-control"></div>
                                <div class="mb-2"><label>Carga Horária</label><input id="edit_carga" name="carga_horaria" type="number" class="form-control"></div>
                                <div class="mb-2"><label>Cargo</label><input id="edit_cargo" name="cargo" class="form-control"></div>
                                <div class="mb-2"><label>ID Usuário</label><input id="edit_id_usuario_func" name="id_usuario_func" type="number" class="form-control"></div>
                            </div>
                            <div class="modal-footer">
            ...
            <!-- Filtros de busca -->
            <form method="get" class="row g-2 mb-3">
                <input type="hidden" name="active_tab" value="funcionarios">
                <div class="col-md-4">
                    <input name="f_func_nome" class="form-control" placeholder="Filtrar por nome"
                           value="<?php echo htmlspecialchars($f_func_nome ?? ''); ?>">
                </div>
                <div class="col-md-3">
                    <input name="f_func_cargo" class="form-control" placeholder="Filtrar por cargo"
                           value="<?php echo htmlspecialchars($f_func_cargo ?? ''); ?>">
                </div>
                <div class="col-md-2">
                    <button class="btn btn-outline-primary w-100">Filtrar</button>
                </div>
                <div class="col-md-2">
                    <a href="/admin/painel?active_tab=funcionarios" class="btn btn-outline-secondary w-100">Limpar</a>
                </div>
            </form>

            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    <thead><tr><th>ID</th><th>Nome</th><th>CPF</th><th>Cargo</th><th>Salário</th><th>Carga</th><th>ID Usuário</th><th>Ações</th></tr></thead>
                    <tbody>
                        <?php foreach ($funcionarios ?? [] as $f): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($f['id_funcionario']); ?></td>
                                <td><?php echo htmlspecialchars($f['nome'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($f['cpf'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($f['cargo']); ?></td>
                                <td>R$ <?php echo number_format($f['salario'], 2, ',', '.'); ?></td>
                                <td><?php echo htmlspecialchars($f['carga_horaria']); ?>h</td>
                                <td><?php echo htmlspecialchars($f['id_usuario']); ?></td>
                                <td>
                                    <button type="button" class="btn btn-sm btn-warning" onclick="openEditFunc(<?php echo (int)$f['id_funcionario']; ?>, '<?php echo htmlspecialchars($f['salario']); ?>', '<?php echo htmlspecialchars($f['carga_horaria']); ?>', '<?php echo htmlspecialchars($f['cargo']); ?>', '<?php echo htmlspecialchars($f['id_usuario']); ?>')">Editar</button>
        ...
        <!-- Pagamentos -->
        <div class="tab-pane fade p-3 bg-white rounded shadow-sm" id="pagamentos" role="tabpanel">
            ...
            <div class="mb-4 p-3 bg-light rounded">
                ...
                <form method="get" class="row g-2">
                    <input type="hidden" name="active_tab" value="pagamentos">
                    <div class="col-md-3">
                        <input type="text" name="f_pag_aluno" class="form-control" placeholder="Nome do Aluno" value="<?php echo htmlspecialchars($f_pag_aluno ?? ''); ?>">
                    </div>
                    <div class="col-md-3">
                        <select name="f_pag_status" class="form-select">
                            <option value="">Todos os Status</option>
                            <option value="pendente" <?php echo ($f_pag_status ?? '') === 'pendente' ? 'selected' : ''; ?>>Pendente</option>
                            <option value="concluido" <?php echo ($f_pag_status ?? '') === 'concluido' ? 'selected' : ''; ?>>Concluido</option>
                            <option value="cancelado" <?php echo ($f_pag_status ?? '') === 'cancelado' ? 'selected' : ''; ?>>Cancelado</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <select name="f_pag_metodo" class="form-select">
                            <option value="">Todos os Metodos</option>
                            <option value="cartao" <?php echo ($f_pag_metodo ?? '') === 'cartao' ? 'selected' : ''; ?>>Cartao</option>
                            <option value="pix" <?php echo ($f_pag_metodo ?? '') === 'pix' ? 'selected' : ''; ?>>PIX</option>
                            <option value="boleto" <?php echo ($f_pag_metodo ?? '') === 'boleto' ? 'selected' : ''; ?>>Boleto</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <button type="submit" class="btn btn-primary w-100">Filtrar</button>
                        <a href="<?php echo $_SERVER['PHP_SELF']; ?>?active_tab=pagamentos" class="btn btn-secondary w-100 mt-2">Limpar</a>
                    </div>
                </form>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        <?php foreach ($pagamentos ?? [] as $p): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($p['id_pagamento']); ?></td>
                                <td><?php echo htmlspecialchars($p['nome_aluno'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($p['cpf'] ?? 'N/A'); ?></td>
                                <td>
                                    <span class="badge bg-<?php 
                                        $status = $p['status'] ?? '';
                                        echo ($status === 'concluido' || $status === 'pago') ? 'success' : (($status === 'pendente') ? 'warning' : 'danger');
                                    ?>"><?php echo htmlspecialchars($status); ?></span>
                                </td>
                                <td>R$ <?php echo number_format($p['valor'], 2, ',', '.'); ?></td>
                                <td><?php echo htmlspecialchars($p['metodo_pagamento'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars(date('d/m/Y H:i', strtotime($p['data_pagamento']))); ?></td>
                                <td>
                                    <button class="btn btn-sm btn-secondary" onclick="openEditPagamento(<?php echo (int)$p['id_pagamento']; ?>,'<?php echo htmlspecialchars($p['status'] ?? 'pendente'); ?>')" title="Atualizar Status">Editar</button>
                                    <form method="post" style="display:inline" onsubmit="return confirm('Remover este pagamento?');">
                                        ...
                                        <input type="hidden" name="id_pagamento" value="<?php echo (int)$p['id_pagamento']; ?>">
                ...
                <?php if (empty($pagamentos)): ?>
                    <p class="text-center text-muted mt-3">Nenhum pagamento encontrado.</p>
                <?php endif; ?>
            </div>

            <!-- Modal de Edicao de Pagamento -->
            <div id="editPagamentoModal" class="modal" tabindex="-1">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form method="post">
                            <input type="hidden" name="action" value="edit_pagamento">
                            <input type="hidden" id="edit_pag_id" name="id_pagamento" value="">
                            <div class="modal-header">
                                <h5 class="modal-title">Editar Status do Pagamento</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label for="edit_pag_status" class="form-label">Status</label>
                                    <select id="edit_pag_status" name="status" class="form-select" required>
        ...
        <!-- Tickets -->
        <div class="tab-pane fade p-3 bg-white rounded shadow-sm" id="tickets" role="tabpanel">
            <h3>Gerenciar Tickets de Suporte</h3>

            <!-- Filtros -->
            <div class="mb-4 p-3 bg-light rounded">
                <h5 class="mb-3">Filtros</h5>
                <form method="get" class="row g-2">
                    <input type="hidden" name="active_tab" value="tickets">
                    <div class="col-md-3">
                        <input type="text" name="f_ticket_aluno" class="form-control" placeholder="Nome do Aluno" value="<?php echo htmlspecialchars($f_ticket_aluno ?? ''); ?>">
                    </div>
                    <div class="col-md-3">
                        <select name="f_ticket_status" class="form-select">
                            <option value="">Todos os Status</option>
                            <option value="aberto" <?php echo ($f_ticket_status ?? '') === 'aberto' ? 'selected' : ''; ?>>Aberto</option>
                            <option value="em_andamento" <?php echo ($f_ticket_status ?? '') === 'em_andamento' ? 'selected' : ''; ?>>Em Andamento</option>
                            <option value="resolvido" <?php echo ($f_ticket_status ?? '') === 'resolvido' ? 'selected' : ''; ?>>Resolvido</option>
                            <option value="fechado" <?php echo ($f_ticket_status ?? '') === 'fechado' ? 'selected' : ''; ?>>Fechado</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <input type="text" name="f_ticket_categoria" class="form-control" placeholder="Categoria" value="<?php echo htmlspecialchars($f_ticket_categoria ?? ''); ?>">
                    </div>
                    <div class="col-md-3">
                        <button type="submit" class="btn btn-primary w-100">Filtrar</button>
                        <a href="<?php echo $_SERVER['PHP_SELF']; ?>?active_tab=tickets" class="btn btn-secondary w-100 mt-2">Limpar</a>
                    </div>
                </form>
            </div>

            <!-- Tabela de Tickets -->
            <h5>Todos os Tickets</h5>
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <?php foreach ($tickets ?? [] as $t): ?>
                            <tr>
                                <td><code><?php echo htmlspecialchars(substr($t['ticket'], 0, 12)); ?>...</code></td>
                                <td><?php echo htmlspecialchars($t['nome'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($t['categoria_suporte'] ?? 'N/A'); ?></td>
                                <td>
                                    <span class="badge bg-<?php 
                                ...
                                <td><?php echo htmlspecialchars(substr($t['descricao_suporte'], 0, 50)); ?>...</td>
                                <td>
                                    <a href="/ticket?ticket=<?php echo urlencode($t['ticket']); ?>" class="btn btn-sm btn-primary" title="Ver Detalhes">Ver</a>
                                    <form method="post" style="display:inline" onsubmit="return confirm('Remover este ticket?');">
                                        <input type="hidden" name="action" value="delete_ticket">
                                        <input type="hidden" name="ticket_id" value="<?php echo htmlspecialchars($t['ticket']); ?>">
                                        <button type="submit" class="btn btn-sm btn-danger">Remover</button>
                                    </form>
                                </td>
                            </tr>
                ...
                <?php if (empty($tickets)): ?>
                    <p class="text-center text-muted mt-3">Nenhum ticket encontrado.</p>
                <?php endif; ?>
            </div>
        </div>
...
function openEditPagamento(id, status){
    document.getElementById('edit_pag_id').value = id;
    document.getElementById('edit_pag_status').value = status;
    var modal = new bootstrap.Modal(document.getElementById('editPagamentoModal'));
    modal.show();
}
...
function openEditFunc(id, salario, carga, cargo, idUsuario) {
    ...
    document.getElementById('edit_id_usuario_func').value = idUsuario;
